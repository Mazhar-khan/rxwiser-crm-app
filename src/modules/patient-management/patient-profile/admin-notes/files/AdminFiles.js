...
const AdminFiles = () => {
  ...
  const postFile = async (event) => {
    ...
    try {
      setIsLoading(true);
      const data = await ApiService("post", "admin_files", formPayload);
      getAllFiles();
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  const deleteLocation = async (obj) => {
    try {
      const id = obj.id;
      setIsLoading(true);
      const data = await ApiService("delete", `admin_files/${id}`);
  ...
  const getAllFiles = async () => {
    try {
      ...
      const data = await ApiService(
        "get",
        `admin_files/patient/${patient.id}`
      );
      console.log("data", data);
      setGetFiles([]);
      setGetFiles(data);
  ...
  const downloadFile = async (obj) => {
    try {
      const id = obj.id;
      setIsLoading(true);
      const data = await ApiService("get", `admin_files/${id}/download`);
      console.log("data", data);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
